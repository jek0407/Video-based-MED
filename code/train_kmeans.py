...
def select_features(features: List[np.ndarray], k: int = 12) -> np.ndarray:
    """
    ...
    # Concatenating all features into one array [Sum_N x D]
    all_features = np.concatenate(features, axis=0)
    
    # Randomly selecting `num_features_to_select` features
    # indices = np.random.choice(all_features.shape[0], num_features_to_select, replace=False)
    # selected_features = all_features[indices]

    # Select every kth feature
